...
public class KeyMap
{
	...
	protected static KeyMap get(Node n)
	{
		...
		n.addEventHandler(KeyEvent.KEY_PRESSED, (ev) -> handleEvent(m, ev));
		n.addEventHandler(KeyEvent.KEY_RELEASED, (ev) -> handleEvent(m, ev));
		n.addEventHandler(KeyEvent.KEY_TYPED, (ev) -> handleEvent(m, ev));
		return m;
	}
	
	
	protected static void handleEvent(KeyMap m, KeyEvent ev)
	{
		// FIX
		SB sb = new SB();
		if(ev.isAltDown())
		...
		Runnable a = m.actions.get(key(ev));
		if(a != null)
		{
			a.run();
		}
